...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for network in orm.Network.objects.all():
            subnet = IP('%s/%s' %(network.netaddress, network.netmask ) )
            subnet_prefix = subnet.prefixlen()
            network.cidr = '%s/%s' %( network.netaddress, subnet_prefix )
            network.save()
    ...
    def backwards(self, orm):
        ...
        for network in orm.Network.objects.all():
            subnet = IP('%s' %network.cidr )
            netmask = subnet.netmask().strNormal()
            netaddress = subnet.net().strNormal()
