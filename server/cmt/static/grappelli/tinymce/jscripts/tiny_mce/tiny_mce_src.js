...
(function() {
	...
	tinymce.create('tinymce.util.URI', {
		URI : function(u, s) {
			...
			// For example: mailto,skype,tel files.
...
tinymce.util.Quirks = function(editor) {
	...
	function fakeImageResize() {
		...
		// Details about each resize handle how to scale files
		resizeHandles = {
...
tinymce.html.Styles = function(settings, schema) {
	...
	return {
		...
		parse : function(css) {
			...
			// Decodes the specified string by replacing all _<num> with it's original value \" \' files
			...
			function trim(node) {
				...
				if (type != 9) {
					...
					if (type == 3 && node.nodeValue.length > 0) {
						...
						// Keep non empty elements or img, hr files
...
var Expr = Sizzle.selectors = {
	...
	filters: {
		...
		text: function( elem ) {
			...
			// IE6 and 7 will map elem.type to 'text' for new HTML5 types (search, files)
			...
			function normalizeEndPoint(start) {
				...
				function findTextNodeRelative(left, startNode) {
					...
					while (node = walker[left ? 'prev' : 'next']()) {
						...
						// Break if we find a block or a BR/IMG/INPUT files
				...
				if (container === body) {
					// If start is before/after a image, table files
					if (start) {
		...
		return {
			...
			serialize : function(node, args) {
				...
				if (impl.createHTMLDocument) {
					...
					// set the new document in DOMUtils so createElement files works
					oldDoc = dom.doc;
					dom.doc = doc;
				}
...
(function(tinymce) {
	tinymce.dom.ScriptLoader = function(settings) {
		...
		function loadScript(url, callback) {
			...
			// for those at least to help users track their missing plugins files
...
(function(tinymce) {
	...
	tinymce.create('tinymce.Editor', {
		Editor : function(id, settings) {
			...
			// Creates all events like onClick, onSetContent files see Editor.Events.js for the actual logic
			self.setupEvents();
...
(function(tinymce) {
	...
	tinymce.EnterKey = function(editor) {
		...
		function handleEnterKey(evt) {
			...
			// Don't split PRE tags but insert a BR instead easier when writing code samples files
