...
class InterfaceInline(admin.TabularInline):
    model = Interface
    # RB: used to override width of input boxes
    form = InterfaceForm
    exclude = GlobalAdmin.fields
...
class GenericCMTListFilter(SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        ...
        for l in set(l_objects):
            ...
            if hasattr(l, 'label'):
                l_name = l.label
                l_filter = {
                    self.parameter_name + '__label': l.label
                }
                l_count_str = ' (%d)' % len(
                    model_admin.model.objects.filter(**l_filter)
                )
            elif hasattr(l, 'name'):
                l_name = l.name
                l_filter = {
                    self.parameter_name + '__name': l.name
                }
                l_count_str = ' (%d)' % len(
                    model_admin.model.objects.filter(**l_filter)
                )
            lookups.append(
                (l.id, l_name + l_count_str)
            )
        return lookups
    ...
    def queryset(self, request, queryset):
        ...
        """
        q_filter = {
            self.parameter_name + '__id': self.value()
        }
...
class EquipmentAdmin(CMTAdmin):
    fieldsets = (
        ...
        (
            'Machine specifications', {
                'fields': (
                    'specifications', 'warranty', 'warranty_tag', 'serial_number'
                )
            }
        ),
        (
    ...
    list_display = (
        '__unicode__', 'warranty_tag', 'cluster',
        'address', 'room', 'rack', 'first_slot',
        'specifications', 'roles', 'in_support'
    )
